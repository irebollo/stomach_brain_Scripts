function timeseries_statsCluster_Heart_narrow(subjects,cfgMain)
...
filenamePLV = strcat(global_path2root,'\Subjects\','Subject',sprintf('%.2d',subj_idx),'\Timeseries','\Heart\empPLV_heart_narrow_subject_',sprintf('%.2d',subj_idx),'.nii');

% filenamePLVSurrogate = strcat(global_filename(subj_idx,cfgMain,strcat('medianRotationFilename_',cfg.Timeseries2Regress)),'.nii');
filenamePLVSurrogate  = strcat(global_path2root,'\Subjects\','Subject',sprintf('%.2d',subj_idx),'\Timeseries','\Heart\surrPLV_heart_narrow_subject_',sprintf('%.2d',subj_idx),'.nii');
...
for iCluster = 1:NPClusters 
    ...
    tools_writeMri(data,strcat(clusterOutputFilename,'_narrow_map_clusterN',num2str(iCluster)))
end
...
save(strcat(clusterOutputFilename,'_narrow_stats.mat'),'stat')
save(strcat(clusterOutputFilename,'_narrow_summary.mat'),'summaryResults')
...
data = stat.mask;
tools_writeMri(data,strcat(clusterOutputFilename,'_narrow_mask'))
tools_writeMri(tstatCluster,strcat(clusterOutputFilename,'_narrow_tmap'))
tools_writeMri(tXvoxel,strcat(clusterOutputFilename,'_narrow_WholeBrainTmap'))
