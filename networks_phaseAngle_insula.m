...
%obtain insula angles all subjects
for iSubj = 1:length(subjects)
        subj_idx = subjects(iSubj)

networks_phaseAngle_obtain_insula(subj_idx,cfgMain)
end

%% load the angle of each cluster and subject

angleClusters_allsubjects = zeros(30,13);
for iSubj = 1:length(subjects)
    subj_idx = subjects(iSubj)
    ...
    load(clusterTimeseries_insula_phaseAngle_filename)
...
[values indexes] = sort(angle(group_angleCluster),'descend') % sort them according to 
    ...
    colorTable = ...
[0 1 0;0 0.9 0.1; 0 0.8 0.2; 0 0.7 0.3; 0 0.6 0.4; 0 0.55 0.45; 0 0.45 0.55 ; 0 0.4 0.6;0 0.3 0.7; 0 0.2 0.8; 0 0.1 0.9; 0 0 1; 0 0 0]
labels = {'vOcc','dOcc','RCZp','SIr','CCZ','dPrec','PCC','SII l','OccTem','SII r','la dPrec','IPS','Insula'};
...
indexes = repmat([1:13],30)
indexes = indexes (:,1:13)
...
h1 = polar([angle(group_angleCluster(13));angle(group_angleCluster(13))],[zeros(1,1);abs(group_angleCluster(13))])
set(h1,'color',[0 0 0],'linewidth',12)
