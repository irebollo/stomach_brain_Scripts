...
index_sample_begin= find(events_eyelink.dat(1,:) == [events_eyelink.input(indx_markers(1)).timestamp]+1000); % the pupil data is shifted 1s to account for the pupillry response delay -> neural signal of pupil dilation occurs 1s earlier than pupil peak dilation
index_sample_ends= find(events_eyelink.dat(1,:) == [events_eyelink.input(indx_markers(450)).timestamp])+2800; % takes 1.8s after the sample were the volume started + delay. 1.8 isntead of 2s due to resampling to 0.5, if I put 2s i end up with data from next volume (451 instead of 450)
% index_sample_ends= find(events_eyelink.dat(1,:) == [events_eyelink.input(indx_markers(448)).timestamp])+2800; % takes 1.8s after the sample were the volume started + delay. 1.8 isntead of 2s due to resampling to 0.5, if I put 2s i end up with data from next volume (451 instead of 450)


index_sample_begin_allVolumes = zeros(1,450);
for iVolume = 1:450
index_sample_begin_allVolumes(iVolume) = find(events_eyelink.dat(1,:) == [events_eyelink.input(indx_markers(iVolume)).timestamp]+1000);
end
%% Take regressors  per volume

nansXvolume = zeros(1,450);
propotion_nansXvolume =zeros(1,450);
for iVolume = 1:450
nansXvolume(iVolume) = sum(isnan(data_pupil_clean_gaps.trial{1,1}(index_sample_begin_allVolumes(iVolume):index_sample_begin_allVolumes(iVolume)+1999)));
propotion_nansXvolume(iVolume) = sum(isnan(data_pupil_clean_gaps.trial{1,1}(index_sample_begin_allVolumes(iVolume):index_sample_begin_allVolumes(iVolume)+1999)))/2000; 
end

% total proportion of missing data during the scan (more than )
propotion_nan_wholeScan = sum(nansXvolume)/(450*2000);
