function timeseries_statsCluster_Regression_PPC(subjects,cfgMain)
...
% Output filename
  clusterOutputFilename = strcat(global_filename(0,cfgMain,'clusterOutputFilenamePPC'));
...
filenamePLVSurrogate = strcat(global_filename(subj_idx,cfgMain,strcat('medianRotationFilename_',cfgMain.Timeseries2Regress)),'.nii');

    % Load empirical PLV  
    
    PLVGroupEmpirical{iS} = ft_read_mri([PPC_filename '.nii']); % Put into cell
    ...
    PLVGroupSurrogate{iS}.coh = ones(53,63,46).*median(PLVGroupEmpirical{iS}.coh(indInside));
